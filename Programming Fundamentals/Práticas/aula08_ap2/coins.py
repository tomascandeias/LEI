...
def value(bag):
	...
	total = 0
	for key, value in zip(bag.keys(), bag.values()):
		total += key*value
	return total
...
def transfer1coin(bag1, c, bag2):
	...
	if c in bag1.keys() and bag1[c] != 0:
		if c in bag2.keys():
			bag1[c] -= 1
			bag2[c] += 1
		else:
...
def transfer(bag1, amount, bag2):
	...
	for coin in COINS:
		...
		if bag1.get(coin, 0) < to_transfer[coin]: #se bag1 não tem moedas suficientes do tipo "coin"
	...
	for coin, number in to_transfer.items():
